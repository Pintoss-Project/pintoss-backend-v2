...
*   요구사항
*   1. 빈 값 혹은 null값은 안된다.
*   2. 영문+숫자+특수문자, 8~20자로 구성되며 특수문자는 필수이다.
*/
@Embeddable
...
public class Password {
    ...
    private void validate(String value) {
        if (value == null || value.isBlank()) {
            throw new EmptyPasswordException("비밀번호는 필수 값 입니다.");
        }
        if (!PASSWORD_PATTERN.matcher(value).matches()) {
            throw new InvalidPasswordFormatException("영문, 숫자, 특수문자를 포함한 8~20자의 비밀번호를 입력해주세요.");
        }
    }
